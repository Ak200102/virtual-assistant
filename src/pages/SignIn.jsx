...
const SignIn = () => {
    ...
    return (
        <div className='w-full h-[100vh] bg-center bg-no-repeat bg-cover flex justify-center items-center' style={{ backgroundImage: `url(${bg})` }}>
            <form className='w-[90%] h-[600px] max-w-[500px] bg-[#00000000] backdrop-blur hover:shadow-lg  shadow-black flex flex-col items-center justify-center gap-[20px] px-[20px] rounded-xl transition-all duration-300 ease-in-out'onSubmit={handleSignIn}>
                <h1 className='text-white text-3xl font-semibold mb-3'>Sign In to <span className='text-blue-400'>Virtual assistant</span></h1>
                ...
                <button className='min-w-37 h-15 text-black text-xl mt-8 cursor-pointer bg-white hover:bg-black/20 hover:text-white hover:shadow-xl shadow-black  rounded-full transition-all duration-300 ease-in-out' disabled={loading}>{loading?"Loading...":"Sign In"}</button>
                <p className='text-white text-xl'>Want to create a new account ? <span className='text-blue-400 underline cursor-pointer' onClick={()=>navigate("/signup")}>Register</span></p>
            </form>
        </div>
    )
}
